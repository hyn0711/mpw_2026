// Remove ZP instr.
// Update the cycle (load mode, send input data)
...
module pim_dma_v3 #(
	...
	localparam PIM_DEBUG    = 3'b100;   // PIM_DUBUG
    ...
    localparam int SIZE_LOAD            = 8;
    localparam int SIZE_DEBUG_PARALLEL  = 16;
    localparam int SIZE_DEBUG_RBR       = 8;
        ...
        if (rst_ni == '0) begin
            ...
            if (operation_start) begin
                case (funct3_i)
                    ...
                    PIM_LOAD: size <= SIZE_LOAD;    // fixed 8 cycles for LOAD
                    default: size <= '0;
                endcase
            end else begin
                size <= size;
    ...
    always @(*) begin
        ...
        case (funct3)
            ...
            PIM_DEBUG: pim_read_addr = PIM_BASE_ADDR;
    ...
    always_comb begin
        rev_trans_counter = '0;
        case (funct3)
            PIM_PARALLEL: rev_trans_counter = 3 - trans_counter;
    ...
    always_comb begin
        case (curr_state)
            ...
            RW_SETUP: begin
                if (bus_gnt_i) begin
                    if ((funct3 == PIM_ERASE) || (funct3 == PIM_PROGRAM) || (funct3 == PIM_PARALLEL) || (funct3 == PIM_RBR) || (funct3 == PIM_READ)) begin
                        if (pim_valid) begin
                            next_state = MODE_EXE;
                        end else begin
                            next_state = RW_SETUP;
            ...
            RW_EXE: begin
                ...
                //     if ((funct3 == PIM_LOAD) || (funct3 == PIM_DEBUG)) begin
                //         next_state = W_EXE;
                //     end else begin
                //         next_state = IDLE;
                //     end
            ...
            W_EXE: begin
                if (!bus_gnt_i) begin
                    next_state = W_EXE;
                end else if (!trans_running) begin
                    next_state = IDLE;
                end else begin
                    next_state = W_EXE;
                end
    ...
    always_comb begin
        ...
        dma_wr_data_1_o = '0;
        case (curr_state)
            RW_SETUP: begin     // Check the PIM Status
                dma_busy_o = 1'b1;
                bus_req_o = 1'b1;
                count_start = 1'b0;
            ...
            RW_EXE: begin
                ...
                if (bus_gnt_i) begin  
                    ...
                    if (funct3 == PIM_LOAD || funct3 == PIM_DEBUG) begin
                        ...
                        dma_size_1_o = 4'b1111;
                        dma_wr_data_1_o = '0;
                    end else begin      // Parallel & RBR mde (Read sram data and send to PIM)
                        dma_addr_0_o = mem_addr;
                        dma_write_0_o = 1'b0;
                        dma_read_0_o = 1'b1;
                        dma_size_0_o = 4'b1111;
            ...
            W_EXE: begin
                ...
                if (bus_gnt_i) begin   
                    cnt_decr = 1'b1;    
                    mem_incr = 1'b1;
                    if ((funct3 == PIM_LOAD) || (funct3 == PIM_DEBUG) || (funct3 == PIM_READ)) begin
                        dma_addr_0_o = mem_addr;
                        dma_write_0_o = 1'b1;
                        dma_read_0_o = 1'b0;
                        dma_size_0_o = 4'b1111;
